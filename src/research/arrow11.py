...
# Six short-only rings around Arrow 10 B/short_hot_or15|ema. No longs. No 5m ORBR. No HH/HL.
IDS = (
    "short_hot|control",
    "short_hot|gap1",
    "short_hot|or25",
...
POPS = {
    "short_hot|control": {"dv_min": 0.80, "gap_min": 0.02, "or_w_min": 0.04},
    "short_hot|gap1": {"dv_min": 0.80, "gap_min": 0.01, "or_w_min": 0.04},
    "short_hot|or25": {"dv_min": 0.80, "gap_min": 0.02, "or_w_min": 0.025},
    "short_hot|dv60": {"dv_min": 0.60, "gap_min": 0.02, "or_w_min": 0.04},
    "short_hot|two_close": {"dv_min": 0.80, "gap_min": 0.01, "or_w_min": None},
    "short_hot|vwap": {"dv_min": 0.80, "gap_min": 0.015, "or_w_min": None},
}
...
def _replay_one(args: tuple) -> dict:
    ...
    for sym, sdf in by_sym.items():
        ...
        in_pop = {
            n: short_kernel_pop(rank, gap, or_w, **POPS[n]) for n in IDS
        }
        if not any(in_pop.values()):
            continue

        or_break = or15_break_short(sdf) if any(
            in_pop[n] for n in ("short_hot|control", "short_hot|gap1", "short_hot|or25", "short_hot|dv60")
        ) else []
        two = two_close_below_or_low(sdf) if in_pop["short_hot|two_close"] else []
        vw = vwap_short_after_or(sdf) if in_pop["short_hot|vwap"] else []

        def _keep_short_ema(sigs: list) -> list:
            out = []
            for sig in sigs:
                if sig.side != -1:
                    continue
        ...
        for n in ("short_hot|control", "short_hot|gap1", "short_hot|or25", "short_hot|dv60"):
            if in_pop[n]:
                buckets[n].extend(_keep_short_ema(or_break))
        if in_pop["short_hot|two_close"]:
            buckets["short_hot|two_close"].extend(_keep_short_ema(two))
        if in_pop["short_hot|vwap"]:
            buckets["short_hot|vwap"].extend(_keep_short_ema(vw))
...
def run_arrow11(*, workers: int | None = None) -> int:
    ensure_dirs()
    ...
    print(
        f"research start mode=arrow11 workers={workers} cpu={cpu} "
        f"develop={develop[0]}..{develop[-1]} holdout={holdout[0]}..{holdout[-1]}",
        flush=True,
    )
    print(
        "kernel=B/short_hot_or15|ema; short only; concentric pop/entry rings; flatten 11:59. No Arrow 12.",
        flush=True,
    )
    ...
    prog = Progress(len(jobs), "arrow11")
...
def _write_reports(rows, workers, cpu, develop, holdout, capped) -> None:
    ...
    if promo:
        ...
        verdict = (
            "VERDICT: FAIL — no Arrow 11 book has holdout >= $200/day AND non-red develop. "
            "Develop-red / holdout-green is not a pass."
        )

    ctrl = {trk: next(r for r in results if r["track"] == trk and r["name"] == "short_hot|control") for trk in ("A", "B")}
    ...
    diluted = []
    for r in results:
        if r["name"] == "short_hot|control":
            continue
        if r["holdout"]["per_day"] < 0 or r["develop"]["per_day"] < 0:
            diluted.append(f"{r['track']}/{r['name']}")

    lines = [
        "Arrow 11 — concentric expansion of B/short_hot_or15|ema (short only)",
        verdict,
        ...
        "Kernel: short only, Q5, gap-down>=2%, OR width>4%, 15m downside break, ema15 short-stack, flatten 11:59.",
        "No long mirror. No 5-min ORBR. No HH/HL stack. No Arrow 12.",
        "",
    ...
    lines.append("")
    lines.append(
        f"{'track':<6} {'id':<22} {'dev $/day':>10} {'hold $/day':>11} {'hold n':>7} {'hit':>6} {'avgR':>7} {'maxDD':>10} {'>=200':>6}"
    )
    for r in results:
        h = r["holdout"]
    ...
    (REPORTS / "arrow11_results.txt").write_text(text, encoding="utf-8")
    print(text, flush=True)

    log_path = REPORTS / "RESEARCH_LOG.md"
    stamp = datetime.now(timezone.utc).astimezone(ET).isoformat(timespec="seconds")
    dil_txt = ", ".join(diluted) if diluted else "none (or none went red)"
    bits = [
        f"## {stamp} — Arrow 11",
        "",
        verdict,
        "",
        "Kernel: Arrow 10 B/short_hot_or15|ema (develop +$5.58, holdout +$21.53, 58/16 trades). "
        "Short only. Did not mirror into longs. Did not rerun 5-min ORBR. Did not stack HH/HL. "
        "Six rings: control on both tracks; gap>=1%; OR>2.5%; dv>=0.60; two-close below OR low; "
        "first close below RTH VWAP. Flatten 11:59.",
        "",
        "Rings that diluted the kernel (develop red and/or holdout red): " + dil_txt + ".",
        "",
        "n_vs_control:",
    ]
    for r in results:
