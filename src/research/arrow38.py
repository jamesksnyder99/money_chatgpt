"""Arrow 38 — premarket-high reclaim after the open wash. Do not rerun 36 or 37."""
...
from research.signals import MINUTE_1000, MINUTE_1159, MINUTE_1559, RTH_OPEN, bar_time
from research.split import develop_holdout
from research.strategies38 import (
    DIP_CTRL,
    DIP_LOOSE,
    RVOL_SENS,
    _min_low_by_1000,
...
FLAT_1159 = {"flatten_at": MINUTE_1159, "last_entry_at": MINUTE_1159}
TRAIL_1559 = {
...
EXPERIMENTS = (
    ("pmrecl|dip3|flat1159", DIP_CTRL, None, FLAT_1159),
    ("pmrecl|dip3|atr1559", DIP_CTRL, None, TRAIL_1559),
    ("pmrecl|dip2|flat1159", DIP_LOOSE, None, FLAT_1159),
    ("pmrecl|dip3|rvol3", DIP_CTRL, RVOL_SENS, FLAT_1159),
)
...
def _maybe_candidate(pack: dict, pc: float) -> bool:
    ...
    if pm_h is None or o930 is None or o930 >= pm_h - 1e-12:
        return False
    wash = _min_low_by_1000(pack)
    if wash is None:
        return False
    return wash <= o930 * (1.0 - DIP_LOOSE) + 1e-12
...
def _scan_one(df: pl.DataFrame, pc: float, prior_dv: float) -> dict | None:
    ...
    return {
        "today_vol": today_vol,
        "cumdv": cumdv,
        "idx_1000": minute_idx(i1000) if i1000 is not None else None,
...
def _replay_one(args: tuple) -> dict:
    ...
    for h in names:
        ...
        if pk is not None:
            packs[h["symbol"]] = pk
    rows = []
    for name, dip, rvol_min, extra in EXPERIMENTS:
        sigs = []
        for h in names:
            sdf = bars.get(h["symbol"])
            ...
            if sdf is None or pack is None:
                continue
            score = float(h["rvol_1000"] if h.get("rvol_1000") is not None else 0.0)
            for s in pm_reclaim_long(
                sdf,
                pack,
                h["prior_close"],
                dip_frac=dip,
                rvol=h.get("rvol_1000"),
                rvol_min=rvol_min,
                score=score,
                tag=name,
            ):
...
def run_arrow38(*, workers: int | None = None) -> int:
    ensure_dirs()
    ...
    print(
        f"research start mode=arrow38 workers={workers} cpu={cpu} "
        f"develop={develop[0]}..{develop[-1]} holdout={holdout[0]}..{holdout[-1]} "
        f"tape={FULL_BARS}",
        flush=True,
    )
    print(
        "premarket-high reclaim after open wash. Do not rerun 36 or 37. "
        "No cell. No score gate. No B-short rescore. No $500/idea. No virgin pull. "
        "Combined holdout is not EV. No Arrow 39.",
        flush=True,
    )
    ...
    print(f"scan pm-reclaim candidates n={len(all_sess)}", flush=True)
    feat: dict[str, dict[str, dict]] = {}
    prog = Progress(len(all_sess), "arrow38_scan")
    ...
    for d in all_sess:
        ...
        for st in feat.get(iso, {}).values():
            ...
            if not st.get("maybe"):
                continue
            r1000 = _rvol_at_idx(cumdv_hist, sym, iso, cal_isos, st.get("idx_1000"))
            cand[iso].append(
                {
                    "symbol": sym,
    ...
    print(f"replay pm-reclaim ids {len(replay_jobs)} sessions x {len(IDS)}", flush=True)
    prog2 = Progress(len(replay_jobs), "arrow38_replay")
...
def _write_report(chunks, b_chunks, iwm, workers, cpu, develop, holdout) -> None:
    ...
    if promo:
        ...
        verdict = (
            "VERDICT: FAIL — no Arrow 38 reclaim book has holdout >= $200/day AND non-red develop "
            "(ids with develop n < 40 are SHELVED). Combined holdout is not expected value (EV)."
        )
    honesty = (
        "Premarket-high reclaim after the open wash. Did not rerun 36 birth or 37 noon hold. "
    ...
    lines = [
        ...
        "Did not rerun 36 birth or 37 noon hold. No cell. No score gate. No B-short rescore. "
        "No $500/idea. No virgin pull. No Arrow 39.",
        ...
        "Population: prior_close [$3,$20], >=5 prior sessions, confirmed launch before 09:30, "
        "09:30 open below 04:00-09:29 high, wash by 10:00. Reclaim = first 1-min close above that high after 10:00. "
        "Cap8, $200/idea, harness floor. Long only. No RVOL floor on control. SHELVE if develop n<40.",
        "",
    ...
    if best is not None and comb_d is not None and comb_h is not None:
        lines.append(
            f"COMBINED {B_LOCK_ID} + {best['name']}  (A33 B lock reprint + best develop-not-red reclaim)"
        )
        lines.append(
        ...
        lines.append(
            f"COMBINED {B_LOCK_ID} + {A31_FLUSH_ID}  (A33 B lock reprint + A31 flush control; no reclaim id qualified)"
        )
    ...
    (REPORTS / "arrow38_results.txt").write_text(text, encoding="utf-8")
    ...
    bits = [
        ...
        verdict,
        "",
        "Premarket-high reclaim after the open wash. Did not rerun 36 or 37. "
        "No cell. No score gate. No B-short rescore. No $500/idea. No virgin pull. "
        "Combined holdout is not EV. No Arrow 39.",
        honesty,
        "",
