...
**Parallelism / worker policy:** maximize useful concurrency without sacrificing determinism or vendor/source limits. At the start, inspect available CPU/logical cores and the existing script worker controls. For ThetaData acquisition, never exceed eight concurrent requests across the entire process tree (or any lower observed vendor limit); use all eight when there is a sufficiently large independent request queue. For CPU/I/O-heavy local ranking, summary loading, validation, replay, horizon scoring and tests, use the largest practical worker count that improves wall-clock time without memory thrash or duplicate writes; benchmark one representative batch if uncertain rather than defaulting blindly to one worker. Primary-source corporate-action research may be split into independent symbol/event batches and worked in parallel when the environment supports it, but each result must carry its own source/date/factor evidence and must be merged deterministically before reranking. Do not parallelize dependent rerank iterations, shared-file writers, freeze decisions, or IS→OOS reveal steps. Report the worker/concurrency choices, any benchmark used, and bottlenecks in the command log.
